...
def generate_launch_description():
    ...
    robots = [
        {'name': 'bot1', 'x_pose': '-1.5', 'y_pose': '-0.5', 'z_pose': 0.01},
        {'name': 'bot2', 'x_pose': '-1.5', 'y_pose': '0.5', 'z_pose': 0.01},
       ]
    # Node for sending and recieving the goal to the robots
    goal_reciever_sender = Node(
            package='multibot_controller',
            executable='goal_sender.py',
            name='goal_sender',
            output='screen',
    )
    # Node for sending the position of a robot
    pose_server = Node(
    package='multibot_controller',
    executable='pose_server.py',
    name='pose_server',
    output='screen'
    )
    ...
    multibot_controller = get_package_share_directory('multibot_controller')
    ...
    urdf = os.path.join(
        multibot_controller, 'urdf', 'turtlebot3_' + TURTLEBOT3_MODEL + '.urdf'
    )

    world = os.path.join(
        get_package_share_directory('multibot_controller'),
    ...
    ld.add_action(pose_server)
    ld.add_action(goal_reciever_sender)
    ...
    for robot in robots:
        ...
        # Spawing the individual turtles
        spawn_turtlebot3_burger = Node(
            package='gazebo_ros',
            executable='spawn_entity.py',
            arguments=[
                '-file', os.path.join(multibot_controller,'models', 'turtlebot3_' + TURTLEBOT3_MODEL, 'model.sdf'),
                '-entity', robot['name'],
        ...
        if last_action is None:
            # Calling add_action directly for the first robot to facilitate chain instantiation via RegisterEventHandler
